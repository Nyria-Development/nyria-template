...
class Dropdown(nextcord.ui.Select):
    def __init__(self, command_list, list_of_modules, depth):
        ...
        if self.depth == "modules":
            ...
            options.append(nextcord.SelectOption(label=str(
                [command['module'] for command in self.command_list if command['command_name'] == self.depth][0])))
        super().__init__(placeholder=self.depth, min_values=1, max_values=1, options=options)

    async def callback(self, interaction: nextcord.Interaction):
        self.depth = str(self.values[0])
        if self.depth == "Home":
            await interaction.edit(embed=self.get_embed(),
                                   view=DropdownView(self.command_list, self.list_of_modules, "modules"))
        else:
            await interaction.edit(embed=self.get_embed(),
                                   view=DropdownView(self.command_list, self.list_of_modules, self.depth))
    ...
    def get_embed(self):
        if any(module == self.depth for module in self.list_of_modules):
            ...
            [command['command_name'] for command in self.command_list if command['command_name'] == self.depth][0])
            help_embed.add_field(name="Command Description:", value=
            [command['description'] for command in self.command_list if command['command_name'] == self.depth][0])
            parameters = [command['options'] for command in self.command_list if command['command_name'] == self.depth][
                0]
            parameter_list = []
            if parameters:
...
class CommandList(commands.Cog):
    ...
    @nextcord.slash_command(
        name="pyrio-list-commands",
        description="List of all commands. Seconde Function",
        force_global=True
    )
